        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-5">
            <div>
              ...
              <Input className="mt-2" placeholder="PT Bank Central Asia Tbk" value={company} onChange={(e) => setCompany(e.target.value)} />
            </div>
            <div>
              <Label className="text-xs uppercase tracking-wider text-muted-foreground">Tanggal Rilis (opsional)</Label>
              <Input type="date" className="mt-2" value={date} onChange={(e) => setDate(e.target.value)} />
            </div>
            <Button className="w-full" onClick={() => m.mutate()} disabled={!company || m.isPending}>
              {m.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <TrendingUp className="mr-2 h-4 w-4" />}
...
function EarningsReport({ d }: { d: Awaited<ReturnType<typeof runEarningsBrief>> }) {
  const tone =
    d.decision === "BELI" ? "text-pos border-pos/40" : d.decision === "JUAL" ? "text-neg border-neg/40" : "text-foreground border-border";
  return (
    <div className="space-y-5">
      ...
      <div>
        <div className="mb-2 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Beat / Miss (4Q)</div>
          ...
          {d.beat_miss.map((q) => {
            ...
            return (
              <div key={q.quarter} className="flex flex-1 flex-col items-center gap-1">
                <div className={`w-full rounded-sm ${q.surprise_pct >= 0 ? "bg-pos/70" : "bg-neg/70"}`} style={{ height: `${h}px` }} />
                <div className="font-mono text-[10px] text-muted-foreground">{q.quarter}</div>
                <div className={`font-mono text-[10px] tabular-nums ${q.surprise_pct >= 0 ? "text-pos" : "text-neg"}`}>{q.surprise_pct >= 0 ? "+" : ""}{q.surprise_pct.toFixed(1)}%</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <div className="mb-2 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Bull Case</div>
          <p className="rounded-sm border border-pos/30 bg-pos/5 p-3 text-[12px] leading-relaxed">{d.bull_case}</p>
        </div>
        <div>
          <div className="mb-2 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Bear Case</div>
          <p className="rounded-sm border border-neg/30 bg-neg/5 p-3 text-[12px] leading-relaxed">{d.bear_case}</p>
        </div>
      </div>

      <div>
        <div className="mb-2 font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Segment Mix</div>
