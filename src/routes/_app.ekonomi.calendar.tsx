  ...
  return (
    <div className="space-y-4">
      <div className="border-b border-border pb-4">
        <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Economic</div>
        <h2 className="mt-1 font-serif text-2xl font-semibold tracking-tight md:text-3xl">Economic Calendar</h2>
        <p className="mt-1.5 max-w-3xl text-[12px] leading-relaxed text-muted-foreground">
          Jadwal rilis indikator ekonomi penting. Sumber: BPS, BI, FRED, World Bank (di-ingest oleh admin).
        </p>
      </div>
      ...
      <Card className="rounded-sm border-border">
        <CardHeader className="flex flex-row items-center justify-between border-b border-border py-3">
          <CardTitle className="flex items-center gap-2 text-[13px] font-semibold uppercase tracking-[0.14em]">
            <CalendarDays className="h-4 w-4" /> {q.data?.length ?? 0} event ({range.from} → {range.to})
          </CardTitle>
          <Select value={country} onValueChange={setCountry}>
            <SelectTrigger className="h-8 w-32 rounded-sm text-[12px]"><SelectValue /></SelectTrigger>
            <SelectContent>
              ...
              <TableBody className="text-[13px] tabular">
                ...
                {(q.data ?? []).map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell><Badge variant="outline" className="font-mono text-[10px]">{e.country}</Badge></TableCell>
                    <TableCell>
                      <div className="font-medium">{e.title}</div>
                      <div className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">{e.category}</div>
                    </TableCell>
                    <TableCell>
                      <span className={
                        e.importance === 3 ? "text-neg" : e.importance === 2 ? "text-foreground" : "text-muted-foreground"
                      }>
                        {"●".repeat(e.importance)}
                      </span>
                    </TableCell>
                    <TableCell className="text-right">{e.forecast ?? "—"}{e.unit && e.forecast != null ? ` ${e.unit}` : ""}</TableCell>
                    <TableCell className="text-right text-muted-foreground">{e.previous ?? "—"}</TableCell>
                    <TableCell className={`text-right font-medium ${
                      e.actual != null && e.forecast != null
                        ? Number(e.actual) >= Number(e.forecast) ? "text-pos" : "text-neg"
                        : ""
                    }`}>{e.actual ?? "—"}</TableCell>
